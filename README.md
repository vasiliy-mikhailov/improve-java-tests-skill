...
(Maven or Gradle) project's PIT mutation kill-rate** — find the code its tests *run* but don't
*verify*, and add tests that kill the surviving mutants — **without weakening any existing test**,
...
- **`kill-surviving-mutants`** — run PIT scoped to one logic-dense class, read the surviving mutants,
  and add **append-only** JUnit tests that kill them by asserting the *correct* behaviour; keep a
  kill only if every previously-passing test still passes. Includes a mutator→assertion playbook and
...
cp -r skills/detect-java-version skills/kill-surviving-mutants <your-project>/.openhands/skills/
```
...
`skills/kill-surviving-mutants/SKILL.md` to raise this project's mutation score."*
...
4. **Kill** — add new `@Test` methods that assert the behaviour each survivor breaks (append-only).
5. **Confirm** — re-run PIT; keep only if the killed count rose and the suite stayed green.
